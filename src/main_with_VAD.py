...
def main():
    ...
    vad = EmobaseCNN(model_path="models/vad.onnx")
    ...
    while True:
        ...
        segments = vad.segment(vad_mask, len(signal), settings.audio_config.common.sample_rate)

        if not segments:
            logger.info("🚫 Нет голосовой активности — пропуск")
            continue
